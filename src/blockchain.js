...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let invalidTime = currentTime - messageTime >= fiveMinutes;
            if (invalidTime) return reject('Message older then 5 minutes');

            let messageUnverified = !bitcoinMessage.verify(message, address, signature);
            if (messageUnverified) return reject('Bitcoin message invalid');
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            block ? resolve(block) : reject(new Error('No block found'));
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async (resolve, reject) => {            
            ...
            stars.length > 0 ? resolve(stars) : reject(new Error('No stars found'));
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(block => {
                block.validate().then(validBlock => {  
                    if(!validBlock) {
                        errorLog.push(`Invalid block #${block.height}, hash: ${block.hash}`);
                    }
                });
